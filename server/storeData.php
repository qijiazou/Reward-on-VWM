...
function uploadDemo($TurkCode,$MTurkID,$BonusCondition,$Data, $Quiz){
    $fileName = $TurkCode."_".$MTurkID."_".$BonusCondition."_".$Quiz.".csv";
...
function uploadFull($TurkCode,$MTurkID,$BonusCondition,$ActualBonus,$PickedTrial,$CatchTrialResult,$Data, $IMI){
    ...
    $fileNameIMI = $TurkCode."_".$MTurkID."_".$BonusCondition."_".$ActualBonus."_IMI.csv";
    $myfileIMI = fopen("IMIData/".$fileNameIMI, "w") or die("Unable to open file!");
    foreach ($IMI as $fields) {
        fputcsv($myfileIMI, $fields);
    }
    fclose($myfileIMI);
    serverSideResponse("OK: 201");
}
...
if (is_ajax()) {
    ...
    if (
        ...
        switch($type) { //Switch case w.r.t. DEMO or FULL version
            case "DEMO":
                uploadDemo($_POST["turkCode"], $MTurkID, $_POST["BonusCondition"], $trials, $_POST["quizAnswer"]);
                break;
            case "FULL":
                if(
                    ...
                    isset($_POST["CatchTrialResult"]) && !empty($_POST["IMI"])
                ){

                    //serverSideResponse("OK: 201");
                    uploadFull($_POST["turkCode"], $MTurkID, $_POST["BonusCondition"], $_POST["ActualBonus"],
                        $_POST["PickedTrial"],$_POST["CatchTrialResult"], $trials, json_decode($_POST["IMI"]));
                }else{
